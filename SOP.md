...
### REPL appears frozen mid-session (resolved in v0.19.5 — different bug from the startup freeze above)

If `orch` was working fine, you switched to another window and back (or just left it idle a while), and now no key does anything — but this is *after* you'd already gotten at least one response, not at the very first prompt — this is a different bug from the v0.19.4 startup freeze:
- **Real cause, confirmed 2026-07-31 via a live goroutine dump** (`kill -QUIT <pid>` on a hung session — this prints all goroutine stacks to stderr and then exits, so only do this once you're giving up on the session anyway): `chzyer/readline`'s `Operation.ioloop()` goroutine — the one that assembles keystrokes into a line, started exactly once per `Instance` with no restart path — had exited permanently and simply wasn't in the dump. `Terminal.ioloop()` (the lower-level raw-rune reader) was still alive and successfully reading keystrokes, but stuck forever trying to hand them to the now-dead `Operation.ioloop()` over an unbuffered channel.
- This traces back to the `7bac4b1` (2026-07-29) EOF-retry fix: it assumes an `io.EOF` from `rl.Readline()` is always a transient App-Nap blip and retries after 200ms. But if the EOF was actually `Operation.ioloop()` dying for good, the retry's `rl.Readline()` call blocks forever on a channel nothing will ever send to again — turning what used to be a clean `bye`-and-exit into a silent, permanent, zero-output hang.
- **Fixed in v0.19.5**: the EOF-retry in `repl.go` now goes through `callReadlineWithTimeout()` (3s bound). If the retry doesn't return in time, `orch` prints a clear message and exits the session cleanly instead of hanging — you'll need to start a new `orch`, but at least you'll know immediately rather than sitting in front of a dead terminal wondering if it's your keyboard.
- **Not yet root-caused**: why the original EOF happens in the first place (what specifically about a window switch causes `Terminal.outchan` or the raw stdin read to produce it). If you hit the new "readline stopped responding" exit message, that's useful signal — please note what you were doing right before it (mid-command? idle? which app did you switch to?) so the actual trigger can be narrowed down next time.
