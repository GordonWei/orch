...
### REPL appears frozen on startup (resolved in v0.19.4 — was misdiagnosed as SIGCONT in v0.19.3)

If `orch` (or `orch --verbose`) prints the backend banner and then just sits there with no prompt, no `🟢 orch ... AI Chief of Staff` line, and no response to any key:
- **Real cause, confirmed 2026-07-31**: if `memory.briefing_source_file` is set, every REPL boot synchronously re-summarizes that file via the local model *before* the REPL loop starts — before any prompt, before the SIGCONT handler even exists. If the local MLX server is wedged (see incident below), this call used to block for the full default 60s `http.Client` timeout with **zero output**, which is indistinguishable from a frozen terminal.
- Confirm with `orch --verbose`: if you never see `[sigcont] received...`, that's expected and not a clue — the process hasn't reached `runREPL()` yet, so that handler was never registered. Look for whether `⏳ generating briefing from ...` printed (added in v0.19.4) instead; if it printed and then nothing for a long time, this is it.
- **Fixed in v0.19.4**: `generateBriefingFromFile`'s model client now has a 20s timeout (was unbounded at the 60s `http.Client` default), and a `⏳ generating briefing from <path>...` line prints before the call so a slow/wedged local model reads as "in progress" instead of silent.
- If the local model itself is the problem: `GET /v1/models` staying healthy does **not** mean generation works — that endpoint doesn't touch the model. Check the actual chat endpoint directly: `curl -X POST <endpoint>/v1/chat/completions -d '{"model":"...","messages":[{"role":"user","content":"hi"}],"max_tokens":5}'`. If that hangs, restart the local server (`mlx_lm.server ...` — use the entrypoint binary, not the deprecated `python -m mlx_lm.server` form, which left a race in one restart attempt during this incident). A wedged MLX process can look alive for a long time: seen here running 12 days with resident memory collapsed to 28MB (a loaded 7B 4-bit model should hold several GB).

### REPL appears frozen mid-session (v0.19.5 bounded it, v0.19.6 actually recovers — different bug from the startup freeze above)

If `orch` was working fine and now no key does anything — but this is *after* you'd already gotten at least one response, not at the very first prompt — this is a different bug from the v0.19.4 startup freeze:
- **Real cause, confirmed 2026-07-31 via a live goroutine dump** (`kill -QUIT <pid>` on a hung session — this prints all goroutine stacks to stderr and then exits, so only do this once you're giving up on the session anyway): `chzyer/readline`'s `Operation.ioloop()` goroutine — the one that assembles keystrokes into a line — had exited permanently and simply wasn't in the dump. `Terminal.ioloop()` (the lower-level raw-rune reader) was still alive and successfully reading keystrokes, but stuck forever trying to hand them to the now-dead `Operation.ioloop()` over an unbuffered channel.
- This traces back to the `7bac4b1` (2026-07-29) EOF-retry fix: it assumes an `io.EOF` from `rl.Readline()` is always a transient App-Nap blip and retries after 200ms. But if the EOF was actually `Operation.ioloop()` dying for good, the retry's `rl.Readline()` call blocks forever on a channel nothing will ever send to again.
- **v0.19.5** bounded that retry (`callReadlineWithTimeout()`, 3s) so it failed visibly instead of hanging forever — but that meant every recurrence auto-exited the session, which is the exact "bye without user intent" problem `7bac4b1` was originally trying to avoid.
- **Fixed properly in v0.19.6**: `readline.Instance` exposes `Terminal` and `Operation` as separate public fields — `Terminal` is confirmed to stay alive and healthy even when `Operation` dies (that's what the goroutine dump showed). `Terminal.Readline()` is the library's own public method for handing back a fresh `Operation` bound to the same still-good `Terminal`. So on timeout, `repl.go` now does `rl.Operation = rl.Terminal.Readline()` and gives the session one more bounded attempt (5s) before finally giving up. Confirmed recurring: same day, right after a normal `DirectChat()` chat-category response — not obviously during a window switch this time, so treat the "App Nap" framing as unconfirmed, not established.
- **Still not root-caused**: why the original EOF happens in the first place. If you ever see "still not responding after rebuilding — exiting this session", that's the last-resort path and genuinely means both the original `Operation` and its rebuild failed within their bounds — note what you were doing right before it (mid-command? idle? just got a response? which app, if any, did you switch to?).
- Do NOT assume this is the same bug as the v0.19.4 startup freeze, and do NOT assume `--verbose`'s `[sigcont]` log is relevant here either — `SIGCONT` was never involved in this failure mode; it was diagnosed from a goroutine dump, not signal telemetry.

### `v0.19.3`'s SIGCONT/App-Nap theory (superseded, kept for history)

The original bug report — REPL unresponsive after switching windows and back — was real, but v0.19.3's fix targeted the wrong cause. `Terminal.KickRead()` on `SIGCONT` was a safety net for an unverified App-Nap theory; the actual repeat incidents were the briefing-generation hang above, unrelated to SIGCONT or readline at all. The `KickRead()`-only handler is still in place (it's cheap and harmless — see `chzyer/readline` race notes below), but don't expect it to fix a startup freeze; use the `v0.19.4` guidance above instead.
- Do NOT try to "fix" `KickRead()` further by adding `ExitRawMode()`/`EnterRawMode()` calls to the SIGCONT handler — that was the original v0.19.3 approach and `go build -race` confirmed it races with readline's own Ctrl+Z/`fg` handling (`Terminal.SleepToResume()`), both touching the unsynchronized `RawMode.state` field in `chzyer/readline`.
