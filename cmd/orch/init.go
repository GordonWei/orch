...
func handleInit() {
	...
	// Step 7: Briefing source file (optional)
	fmt.Println("📋 Startup briefing (optional)")
	fmt.Println("   If you keep a project status/handoff document (e.g. a dashboard you")
	fmt.Println("   update by hand), orch can re-read and summarize it on every startup —")
	fmt.Println("   instead of showing a cached summary that goes stale the moment you")
	fmt.Println("   stop manually running `orch briefing gen`.")
	fmt.Print("   Path to that file (optional, e.g. ~/Desktop/Cowork/docs/_agent_handoff.md): ")
	briefingSourceFile, _ := reader.ReadString('\n')
	briefingSourceFile = strings.TrimSpace(briefingSourceFile)

	fmt.Println()
	fmt.Println("📝 Writing config...")

	// Generate config
	configContent := generateConfig(primary, lang, owner, enableMLX, workspace, mlxModel, briefingSourceFile)
...
func generateConfig(primary, lang, owner string, enableMLX bool, workspace string, mlxModel string, briefingSourceFile string) string {
...
#     default: true`
  ...
  - name: "mlx-default"
	...
	briefingSourceLine := "  # briefing_source_file: \"~/path/to/your/status-or-handoff.md\""
	if briefingSourceFile != "" {
		briefingSourceLine = fmt.Sprintf("  briefing_source_file: %q", briefingSourceFile)
	}
...
keyword_shortcuts:
  ...
  - prefix: "gcloud container"
    category: query
`, ownerLine, lang, strings.ReplaceAll(systemPrompt, "\n", "\n    "), primaryLine, mlxSection, briefingSourceLine, workspaceLine)
}
