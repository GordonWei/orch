...
- Same day as v0.19.4: with that fix installed and the local model confirmed healthy, the REPL still froze mid-session after the user switched windows and back — no key (including Ctrl+C/D) did anything, process didn't exit. Diagnosed live instead of theorizing: `sample` (macOS's stack-sampling tool) on the hung process showed the goroutine reading stdin blocked in a raw `read()` syscall for a full 1s sampling window, not moving — ruled out App Nap/SIGCONT (no network call in flight, OS process state was normal foreground `S+`, not job-control-stopped `T`) and ruled out "wrong window focus" (user confirmed clicking directly into the pane, no tmux/multiplexer). Sent `SIGQUIT` to get a full Go goroutine dump before the process had to be killed anyway.
- **The dump was decisive.** `chzyer/readline`'s two internal goroutines were split: `Terminal.ioloop()` (reads raw runes) was alive, blocked trying to hand a successfully-read character to `Operation.ioloop()` via an unbuffered channel (`terminal.go:210`) — but `Operation.ioloop()` (the goroutine that assembles keystrokes into a line, started exactly once per `Instance` with **no restart path** — `go op.ioloop()` in `operation.go:88`, called once from `NewOperation`) was **not present in the dump at all**. It had already returned, permanently, sometime earlier in the session.
- **Traced this back to `v0.19.3`'s (actually `7bac4b1`, 2026-07-29) EOF-retry fix, and it makes things worse, not better.** That commit's own message says: "When iTerm2 is backgrounded, macOS App Nap can suspend the terminal's PTY, causing Readline() to receive an unexpected io.EOF... wait 200ms and retry once." The retry calls `rl.Readline()` again, which re-enters the same `select` waiting on `Operation`'s `outchan`/`errchan`. If the EOF that triggered the retry was the *permanent* kind — `Operation.ioloop()`'s own EOF-handling path (`operation.go:123-129`) breaks its loop for good when its buffer is empty — the retry has nothing left that will ever answer it. Before `7bac4b1`, this exact condition produced a clean `bye`-and-exit. After it, the same condition hangs forever with **zero output**, which is strictly worse: no diagnostic, no way to tell "read this" from "read nothing, ever again."
- **Fix**: extracted `callReadlineWithTimeout(readFn, timeout)` in `repl.go` — runs `readFn` (in practice `rl.Readline`) in a goroutine and returns `errReadlineTimeout` if it doesn't complete within a bound (3s for the EOF-retry call), instead of blocking forever. On timeout, print a clear message and exit the REPL loop cleanly (matches the pre-`7bac4b1` behavior for this case) rather than hang silently. This doesn't attempt to resurrect the dead `Operation.ioloop()` goroutine — a live `Instance` swap was considered and rejected as too risky to get right without much more testing (this session already found one "obviously safe" concurrency fix, `v0.19.3`'s original `ExitRawMode`/`EnterRawMode` approach, that turned out to have a real `-race`-confirmed bug) — it just guarantees the failure mode is a visible, immediate exit instead of an invisible, permanent hang.
- New tests `TestCallReadlineWithTimeout_Recovers` and `TestCallReadlineWithTimeout_BoundsAHang` (the latter simulates the dead-goroutine case with a `readFn` that blocks on a channel that's never closed, asserting the timeout fires well within the requested bound). `go build` / `go vet` / `go test -race -count=1 ./...` all green (16 packages).
- **What's still open**: the exact trigger for `Operation.ioloop()`'s original EOF — i.e., why `Terminal.outchan` or the raw stdin read produced an EOF in the first place during a window-switch — wasn't fully bisected; this fix bounds the *consequence* (permanent silent hang), not the underlying trigger. If this recurs, the REPL will now exit with a clear message instead of hanging, which is itself useful signal for narrowing the trigger further.

### v0.19.4 (2026-07-31)

**Found the real cause of the recurring "REPL frozen" reports — it was never SIGCONT/App Nap. It was a hung local model call on every boot, with zero visible feedback.**

- Same day as v0.19.3, the freeze recurred with `--verbose` on and **no `[sigcont]` line printed** — the telemetry v0.19.3 added specifically to catch this. That ruled out the SIGCONT path entirely for this occurrence, since a real `[sigcont]` firing without the log line was never possible: the log line sits right next to the only thing the handler does.
- **Actual cause**: `memory.briefing_source_file` (when set — this instance points it at a shared team handoff doc) makes every REPL boot call `generateBriefingFromFile()` synchronously, *before* `runREPL()` — before the prompt, before any input handling, before the SIGCONT handler in v0.19.3 even gets registered. That call's `model.NewOpenAIClient` didn't set a `Timeout`, so it inherited the 60s `http.Client` default. Diagnosed live: the local MLX server had been running 12 days with resident memory collapsed to 28MB (a loaded 7B 4-bit model should hold several GB) — `GET /v1/models` (what `Available()` checks) still returned 200 promptly since it doesn't touch the model, but `POST /v1/chat/completions` never responded at all. Every boot silently ate up to 60s with no output, indistinguishable from a hung terminal — and if the user switched windows during that window (a very natural thing to do while waiting), coming back to "nothing responds" looked exactly like the original App Nap report, despite being a completely unrelated mechanism.
- **Fix**: `generateBriefingFromFile`'s model client now sets `Timeout: 20 * time.Second` (measured: the real ~12,000-char handoff-doc prompt takes ~14s against a healthy local model, so 20s leaves headroom without reintroducing a long silent hang). `main.go` also prints `⏳ generating briefing from <path>...` before the call, so a slow-but-working model reads as progress instead of a stall.
- v0.19.3's `KickRead()`-only SIGCONT handler is left in place — it's cheap, doesn't touch the racy `RawMode.state` field, and the original App-Nap-freeze report might still be a real (separate, unconfirmed) issue. But it's no longer the leading theory for the freeze reports actually seen.
- New regression test `TestGenerateBriefingFromFile_ChatHangs`: a stub server that returns 200 on `/v1/models` but never responds on `/v1/chat/completions`, asserting `generateBriefingFromFile` returns (with an error) well under the old 60s default. `go build` / `go vet` / `go test -race -count=1 ./...` all green (16 packages; new test adds ~20s to the suite by design — it has to wait out the timeout to prove it's bounded).

### v0.19.3 (2026-07-31)

**Kiro's App Nap freeze fix, reviewed — the SIGCONT handler he wrote raced with readline's own Ctrl+Z handling, narrowed it down to the one call that's actually safe.**

- User report: ask orch something, switch to another window, come back, and the REPL is frozen — no key (including Ctrl+C/D) does anything, process doesn't exit. Kiro's theory: macOS App Nap suspends the backgrounded PTY, and on resume `chzyer/readline`'s raw-mode terminal state is stale, so its `ioloop()` goroutine stops delivering runes. His fix: a goroutine listening for `SIGCONT` (delivered when a backgrounded process comes back to the foreground) that calls `Terminal.ExitRawMode()` → `EnterRawMode()` → `KickRead()`.
- **Review caught a real bug in the fix, not just a theoretical one.** `chzyer/readline` already has its own resume handling for Ctrl+Z/`fg` (`Terminal.SleepToResume()`, triggered by `CharCtrlZ` in `operation.go`) — it deliberately does *not* use signal-based resume detection, instead polling a ticker for a >100ms gap. Both `SleepToResume()` and Kiro's new SIGCONT handler call `ExitRawMode()`/`EnterRawMode()`, and both wrap `RawMode.state` (`utils.go`), a bare pointer field with no mutex. Built a standalone reproduction outside this repo (two goroutines hammering the same `Terminal.ExitRawMode()`/`EnterRawMode()` calls readline itself uses) and `go build -race` caught it immediately — real read/write and write/write races on `RawMode.state`, confirmed, not hypothetical. Worst case isn't just "redundant work converges to the same state" as first assumed — a corrupted `state` pointer could make `rl.Close()` restore the terminal to the wrong termios settings on exit, leaving the user's shell needing a manual `reset`. That's worse than the bug being fixed, and it would trigger on a completely ordinary Ctrl+Z/`fg`, not just the rare App Nap case.
- **Also worth flagging: the App Nap→SIGCONT causal chain itself is unverified.** App Nap's usual mechanism is CPU/timer throttling, not POSIX job-control signals — there's no confirmation in Kiro's testing that App Nap actually delivers SIGCONT to a backgrounded PTY child at all. `go build`/`go vet`/`go test -race` all passing doesn't touch this, since nothing exercises the SIGCONT path.
- **Fix applied**: kept the SIGCONT listener but stripped it down to `KickRead()` only — no `ExitRawMode()`/`EnterRawMode()`. `KickRead()` just does a non-blocking send on the buffered `kickChan`, which doesn't touch `RawMode.state`, so it's safe to call from any goroutine regardless of what else is happening. Re-ran the same race reproduction against this version: no warnings. Gated a `[sigcont] received...` log line behind `--verbose` so if the freeze recurs, we'll actually know whether this path fires at all — real telemetry instead of a second unverified guess.
- `go build` / `go vet` / `go test -race -count=1 ./...` all green (16 packages, unchanged from v0.19.2 — no new package-level tests, since the bug only reproduces under real concurrent signal delivery, which the standalone repro (not part of this repo) covers).
...
- **🔴 Fixed: `mlxAvailable()`/`tryMLX()`/`DirectChat()` were making requests through Go's bare `http.Get`/`http.Post`, which means `http.DefaultClient` — no timeout, period.** `mlx_lm.server` has slow moments on `/v1/chat/completions` for no obvious reason — same request, same size, 6s one time and it just never comes back the next. With nothing capping the wait, hitting one of those moments hung the whole orch process forever, no error, nothing — looked exactly like a crash. New `mlxPingClient` (5s) and `mlxChatClient` (60s) replace every bare call. Added a test that the timeouts are actually set, and one that spins up a mock server that never responds and checks `DirectChat()` bails out in ~2s instead of hanging (it shortens the client timeout first so the test itself isn't a minute long).
- **🔴 Fixed: `DirectChat()` had no idea what day it was or what's actually going on in the project.** The `briefing_source_file` feature from v0.16.2 re-summarizes the handoff doc on every boot, but that summary only ever got printed to the terminal — `DirectChat()` never actually saw it, so "what's still pending" got answered by making something up. `Planner.SetBriefing()` now threads the current briefing plus today's date into `DirectChat()`'s system prompt. Also caught: `truncateStr` was slicing by byte length instead of runes, which for a CJK-heavy handoff doc could cut a character in half and hand invalid UTF-8 to the MLX request.
- **🔴 Fixed: `DirectChat()`'s `max_tokens: 2048` meant a reply that never hits a natural stop could run for ~2 minutes** at this machine's local decode speed (~16 tok/s). Dropped to 400 — plenty for a normal chat reply.
- **🔴 Fixed: the routing debug line hardcoded `"MLX local (Qwen 2.5 3B)"` no matter what model was actually running** — still said 3B after the default moved to 7B. `modelDisplayName()` now just reads it off the configured model string.
- **🔴 Fixed: nothing in the classification prompt told the local model to punt on real-time stuff** — weather, news, stock prices — so it would guess `local:chat` and make up an answer instead of routing somewhere that can actually look things up. Added a `claude:query` rule for that.
- Nudged `generateBriefingFromFile`'s prompt to list pending items one by one instead of folding them into a paragraph, since `DirectChat()` reads this directly now and needs to actually enumerate them.
